...
function SectionLabel({ title }: { title: string }) {
  ...
  return <Text style={buildSectionStyle(colors)}>{title}</Text>;
}

const buildSectionStyle = (colors: ColorScheme): object => ({
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isLoanCategory && personName.trim()) {
        if (LOAN_CREATES_NEW_RECORD.has(catName)) {
          loan_id = await createLoan(db, {
            type: LOAN_TYPE_BY_CATEGORY[catName],
            person_name: personName.trim(),
            principal: numericAmount,
            currency,
          });
        } else if (LOAN_IS_REPAYMENT.has(catName)) {
          const openLoan = await findOldestOpenLoan(db, LOAN_TYPE_BY_CATEGORY[catName], personName.trim());
          if (openLoan) {
            loan_id = openLoan.id;
      ...
      await createTransaction(db, {
        ...
        loan_id,
        paid_amount: repaymentAmount ?? (status === 'partial' && !loan_id ? (parseFloat(paidAmount) || 0) : undefined),
